...
resource "azurerm_network_security_group" "security_grp" {
  name                = var.network_security_group_details.name
  location            =  var.resourcegroup_details.location
  resource_group_name =  var.resourcegroup_details.name
...
resource "azurerm_public_ip" "ps4" {
  name                = var.ip_address_details.name
  resource_group_name =  var.resourcegroup_details.name
  location            =  var.resourcegroup_details.location
  allocation_method   = var.ip_address_details.allocation_method
  depends_on = [
    azurerm_resource_group.srii5
  ]
}
...
resource "azurerm_network_interface" "network_interface" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = var.ip_address_details.allocation_method
